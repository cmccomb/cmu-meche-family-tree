...
The script writes two files: ``<basename>.dot`` containing the DOT
source used to build the graph, and ``<basename>.png`` containing the
rendered image.  You must have Graphviz installed on your system to
perform the rendering.  See the Graphviz documentation for
installation instructions【891944500095481†L6-L21】.
...
def render_graph(people: Dict[str, Dict[str, Optional[object]]], edges: Iterable[Tuple[str, str]],
    ...
    """Render the advisor/advisee graph to PNG and DOT files.
    ...
    with open(dot_source_path, "w", encoding="utf-8") as f:
        f.write(dot.source)
    # Graphviz's pipe() method returns binary output of the chosen format
    png_bytes = dot.pipe(format="png")
    with open(png_path, "wb") as f:
        f.write(png_bytes)

    print(f"Saved: {png_path} and {dot_source_path}")
